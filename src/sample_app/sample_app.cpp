...
namespace cpp_practice::cpp_practice_sample_app {

void run_simple_thread(int32_t fibonacci_number) {
  spdlog::debug("Calling fibonacci with number {}", fibonacci_number);
  auto result = cpp_practice::algorithms::fibonacci(fibonacci_number);
...
void run_sample_app(spdlog::level::level_enum console_log_level,
  ...
  auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
      "logs/cpp_practice_sample_app.log", true);
  ...
  auto logger =
      std::make_shared<spdlog::logger>("cpp_practice_sample_app", sinks);
...
int main(int argc, char** argv) {
  CLI::App app{"cpp_practice sample application"};

  app.set_version_flag("--version",
                       cpp_practice::options::kProjectVersion.data());
  ...
  cpp_practice::cpp_practice_sample_app::run_sample_app(
