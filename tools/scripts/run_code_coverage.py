...
def run_tests(inputs: Inputs) -> None:
    ...
    subprocess.run(
        cmake_configure_command, check=True, cwd=ROOT_DIR, text=True)  # nosec

    cmake_build_command = 'cmake --build ./build --parallel 4'
    print(f'Running: {cmake_build_command}')
    subprocess.run(
        cmake_build_command, check=True, cwd=ROOT_DIR, text=True)  # nosec

    cmake_test_command = './cpp_practice_tests --gtest_output=xml'
    print(f'Running: {cmake_test_command}')
    subprocess.run(
        cmake_test_command, check=True, cwd=TEST_DIR, text=True)  # nosec
    ...
    subprocess.run(
        cmake_coverage_command, check=True, cwd=TEST_DIR, text=True)  # nosec
